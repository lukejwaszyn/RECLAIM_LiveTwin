...
authenticated `/ingest` route through Cloudflare, publish authenticated loopback
`/state` through the independent Windows state bridge and existing VM Convene
agent, then hand the ingest URL and ingest token to the Windows 10 gateway owner.

## Ordered session

1. Select and record the exact reviewed source SHA.
2. Discover Windows services, tasks, port 8078, release paths, ACLs, cloudflared,
   durable state, Convene agent identity, and `sim_vars.json` writers before change.
3. Stage the exact revision under
   `C:\ProgramData\RECLAIM\releases\<SHA>` and run locked tests/imports.
4. Configure distinct ingest/read tokens in an ACL-protected Windows secret file;
   never place them in XML, commands, logs, screenshots, or GitHub.
5. Install or reconcile the `RECLAIMIngestEngine` WinSW service, loopback-only,
   production mode, with durable identity under `C:\ProgramData\RECLAIM\engine`.
6. Preserve an existing approved Cloudflare configuration or start a temporary
   Windows quick tunnel to loopback 8078.
7. Run the 20-check acceptance harness using environment-provided credentials and
   prove restart/deduplication persistence.
8. Install the separate `RECLAIMStateBridge` service with the read token only.
9. Bind Convene first to bridge health and lease fields, determine prefix behavior,
   then bind process fields and prove fail-closed lease expiration.
10. Privately hand the Windows 10 gateway owner only the `/ingest` URL, ingest
