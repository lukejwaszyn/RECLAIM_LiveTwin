...
config + current tunnel hostname/token; on-gateway pre-flight (55/67/70 + bench
...
- **"Update to current specifications":** checkout/verify the pinned SHA, run
  ...
  55 / 67 / 70**) and the bench replay, and **refuse to deploy on any red**. Record
  the deployed SHA.
...
- *MacBook side:* `-WhatIf` plan review; `install.py` unit tests;
  `scripts/check_repository_hygiene.py`; full 55/67/70 + bench replay pre-flight.
